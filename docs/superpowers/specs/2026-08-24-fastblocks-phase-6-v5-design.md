...
- *Demonstrable by:*
  ...
  1. `uv sync --group observability` installs; same import succeeds
  1. `.venv/bin/pip show sentry-sdk prometheus-client opentelemetry-sdk opentelemetry-exporter-otlp-proto-http` returns matching versions: prometheus-client 0.21.x, opentelemetry-sdk 1.44.x, opentelemetry-exporter-otlp-proto-http 1.44.x, sentry-sdk 3.0.0a7
  1. `uv tree --depth 1 --group monitoring | grep -E 'sentry|urllib3'` returns 0 hits (consolidation confirmed; urllib3 cleanup)
  1. `uv tree --depth 1 --group observability | grep -E 'grpcio|protobuf|opentelemetry-exporter-otlp-proto-grpc'` returns 0 hits (**Δ23**; proto-grpc NOT pulled)
  1. `[opentelemetry]` extra of `sentry-sdk` includes the OTel auto-instrumentation hooks needed by `sentry_sdk.opentelemetry` import path; not a free extra.
...
- *Demonstrable by:*
  ...
  1. `tests/observability/test_exception_middleware_position.py::test_innermost_opt_out` passes
  1. `tests/observability/test_exception_middleware_position.py::test_otel_outermost_with_5xx` passes (handler raises; OTel root span records exception; span tree is intact)
...
- *Demonstrable by:*
  ...
  1. App lifespan shutdown invokes `provider.shutdown()` (regression test asserts `provider._active_span_processor._shutdown_called is True`)
  1. `tests/htmx/test_trace_context_propagation.py::test_context_survives_executor_boundary` passes (already-shipped regression-preservation per Phase 6.5 commit `5c919f4`)
  1. Sentry import order documented in module docstring (TracerProvider first, sentry_init last)
- *Rollback signal:* `git revert`
...
- *Demonstrable by:*
  ...
  1. Unit test triggers Oneiric resolution; SpanProcessor emits structlog line and increments `fastblocks_oneiric_decision_total{domain, decision="resolved"}` for normal path; same counter increments for `decision="error"` if `traced_decision()` body raises
  1. Unit test fires a non-`resolver.decision` span (e.g., from `opentelemetry-instrumentation-httpx`); **counter is NOT incremented** (Δ8 scope filter)
...
- *Demonstrable by:*
  ...
  1. `cardinality_mode="audit"` lets the metric increment but always emits `fastblocks_cardinality_violations_total{label}` (Δ7)
  1. `cardinality_mode="off"` increments metric with raw value (dev convenience)
- *Rollback signal:* `git revert`
...
- *Demonstrable by:*
  ...
  1. A tool raising a `pydantic.ValidationError` increments `{..., status="validation_error"}` (Δ6 extension)
  1. `Histogram("fastblocks_mcp_tool_duration_seconds", labelnames=("tool_name",), buckets=(0.001,...,5.0)).observe(elapsed, exemplar={"trace_id": trace_context.get().trace_id if trace_context.get() else "0" * 32})`
- *Rollback signal:* `git revert`
...
- *Demonstrable by:*
  ...
  1. `curl -H "Accept: text/plain; version=0.0.4" :3035/metrics` returns `text/plain; version=0.0.4; charset=utf-8` (Δ9 legacy scraper)
  1. Named counter names appear in both formats (verified via `grep -E '^# HELP fastblocks_'`)
- *Rollback signal:* `git revert`
...
- *Demonstrable by:*
  ...
  1. `token = set(TraceContext(trace_id="0"*32, span_id="0"*16))`; `get()` returns the value
  1. `reset(token)`; `get()` returns None again
...
- *Demonstrable by:*
  ...
  1. `MiddlewareManager.get_middleware_stack()["user_middleware"][0]["class"] == "OtelMiddleware"` (Δ resolved: `MiddlewareManager.get_middleware_stack` is a `dict[str, Any]` per `applications.py:114-124`; user_middleware[0] is outermost user middleware by Starlette's reverse-list wrapping)
  1. NEW `tests/observability/test_otel_middleware_outermost.py` confirms OtelMiddleware is `user_middleware[0]` AND that a handler raising produces an OTel root span with `http.response.status_code == 500` (5xx coverage)
  1. `try/finally` clears `trace_context` even on handler exception (regression test)
- *Rollback signal:* `git revert`
...
- *Demonstrable by:*
  ...
  1. Without `SENTRY_DSN`, no-op (no `sentry_init` call)
  1. With `sentry_sdk.opentelemetry` import raising at startup AND `disabled_on_import_error: false` (Δ11 default), app startup fails loud with `RuntimeError`
  1. With `disabled_on_import_error: true` (operator opt-in soft-fail), `fastblocks_sentry_disabled_total{reason="import_error"}` increments; app continues
  1. With `profiling_enabled=True`, **app startup fails loud** with documentation pointing at `observability.sentry.profiling_enabled: bool = false` (Δ20)
...
- *Demonstrable by:*
  ...
  1. **Dynamic Playwright integration test** (Δ16): boots app + WebSocket adapter; fires a known WS broadcast event (e.g., `{type: "render", component: "button", payload: "submit"}`); asserts the matching aria-live text appears with the correct class + computed style; **this test fails if the bridge is unwired even though the static DOM and CSS look correct**
  1. Computed style assertions use cross-browser regex `/^inset\(50%(\s+50%(\s+50%(\s+50%)?)?)?\)$/` (handles Chrome/Firefox/Safari normalization)
  1. Rate-limit test: send 100 events/sec; assert no more than 5 aria-live mutations per second (coalescing); `fastblocks_a11y_bridge_coalesced_total{region}` increments
- *Rollback signal:* `git revert`
...
- *Demonstrable by:*
  ...
  1. Per-dashboard ground-truth: each panel's metric appears in the per-metric instrumentation matrix above
  1. No `TBD` or "future" markers in any panel's `targets[].expr`
- *Rollback signal:* `git revert`
...
12. **Commit 12 dropped** (Δ5): `htmx.py:49-62` already shipped the
    ...
    by item (d).
01. **Commit 0c covers BOTH ExceptionMiddleware sites** (Δ3): line 250
    and line 368-374 must both be fixed; v4 cited only the second.
01. **`ObservabilityRegistry` owned by Commit 1** (Δ15): the
    previously-phantom symbol now has an explicit owner creating
    `registry.py` as a singleton wrapper around
    `prometheus_client.CollectorRegistry`.
01. **`settings/observability.yaml` owned by Commit 0b** (Δ14): the
...
01. **SpanProcessor filter by span name** (Δ8): `on_start` filters to
...
01. **`decision` Literal closed** (Δ6): `Literal["hit","miss","shadowed","error"]`
    replaces the v3 "single value today" framing.
01. **`status` Literal extended** (Δ6): `Literal["ok","error","timeout","validation_error","unauthorized","rate_limited"]`
    captures actual MCP-tool failure modes.
01. **`audit` mode added to CardinalityGuard** (Δ7): counter increments
    but no block; enables SRE workflow off→audit→enforce.
01. **Routing policy corrected** (Δ10): `miss`→polite/status;
    `escaped=false`→logs only (security signal, not user announceable).
01. **`disabled_on_import_error` default flipped to `false`** (Δ11):
    loud-fail default with `fastblocks_sentry_disabled_total{reason}`
    counter for operators who opt into soft-fail.
01. **BatchSpanProcessor shutdown wired in Commit 9** (Δ18): app
    lifespan shutdown handler invokes `provider.shutdown()` to flush
    queued spans.
01. **Sentry init ordering specified** (Δ19): TracerProvider first,
    `sentry_sdk.init` last (per Sentry docs).
01. **`profiling_enabled=False` is the only supported value** (Δ20) when
    bridging OTel; `profiling_enabled=True` fails loud at startup.
01. **Dynamic WS broadcast Playwright test** (Δ16): Commit 13's
...
01. **`opentelemetry-exporter-otlp-proto-http`, not the meta-pkg** (Δ23):
...
01. **`dev` group unchanged; observability is opt-in via `--group`** (Δ24):
...
01. **Breaking change callout for `monitoring` group** (Δ25):
...
01. **Dual-version OTel safety net** (Δ26): if `monitoring` +
...
01. **`urllib3~=2.5` removed from `monitoring`** (P1-2): orphaned by
