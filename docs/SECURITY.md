...
2. **Session Management** - Secure session handling via Starlette
3. **Input Validation** - ACB validation integration (optional)
4. **Secret Detection** - Automated scanning via detect-secrets & gitleaks
5. **Code Security** - Static analysis via Bandit
6. **Secure Headers** - Security headers middleware
7. **Template Safety** - Sandboxed Jinja2 environment
...
2. Never disable CSRF for production deployments
3. Rotate CSRF secrets regularly
4. Use secure, random secrets (32+ characters)
...
3. Enable Secure flag for HTTPS-only cookies
4. Use SameSite=Strict for sensitive applications
5. Rotate session secrets periodically
...
2. **Custom** - Extensible adapter pattern for OAuth, JWT, etc.
...
2. Sanitize output for display
3. Use parameterized queries for database operations
4. Whitelist allowed values when possible
5. Reject invalid input early
...
2. **STRICT** - Strict undefined handling
3. **SANDBOXED** - Full sandbox with tag/attribute restrictions
...
2. Use SANDBOXED mode for user-generated templates
3. Whitelist allowed tags and attributes
4. Review all |safe filter usage
5. Validate template sources
...
2. Configure restrictive CSP policies
3. Review CSP violations regularly
...
2. Review all findings before baseline updates
3. Never commit real secrets to version control
4. Use environment variables for secrets
5. Rotate leaked secrets immediately
...
4. Input validation comprehensive tests
...
async def test_xss_prevention():
    ...
    response = await client.post("/comment", data={
...
2. **A02 Cryptographic Failures** - Session encryption, HTTPS ✓
3. **A03 Injection** - Parameterized queries, input validation ✓
4. **A04 Insecure Design** - Defense in depth architecture ✓
5. **A05 Security Misconfiguration** - Security defaults, headers ✓
6. **A06 Vulnerable Components** - Bandit scanning ✓
7. **A07 Auth Failures** - Secure session management ✓
8. **A08 Data Integrity Failures** - CSRF protection, secure cookies ✓
9. **A09 Logging Failures** - ACB debug logging ✓
10. **A10 SSRF** - Input validation (partial coverage)
...
2. OAuth2 authentication adapter
3. JWT authentication adapter
