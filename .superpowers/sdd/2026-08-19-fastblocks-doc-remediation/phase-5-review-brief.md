...
1. `fastblocks/adapters/app/README.md` — ACB imports rewritten; main.py→default.py; template variants listed
1. `fastblocks/adapters/auth/README.md` — ACB imports rewritten; "Migrated to Oneiric" note added
1. `fastblocks/adapters/fonts/README.md` — Verified (likely just stale-content warning)
1. `fastblocks/adapters/icons/README.md` — Verified
1. `fastblocks/adapters/images/README.md` — Verified; cf_image_url/twicpics helpers spot-checked
1. `fastblocks/adapters/routes/README.md` — ACB imports rewritten; main.py→default.py
1. `fastblocks/adapters/sitemap/README.md` — ACB imports rewritten; 7-file inventory
1. `fastblocks/adapters/style/README.md` — vanilla.py added; phantom bulma.py removed
1. `fastblocks/adapters/templates/README.md` — htmy and hybrid rows added
1. `fastblocks/adapters/README.md` (parent) — 6 missing categories added
1. `fastblocks/mcp/README.md` — "10+ →" → "10"

### Quality

1. **Stale-content warnings**: Each primary file should have the warning banner near the top.
1. **Import verification**: Spot-check 3-5 of the `from oneiric.*` / `from fastblocks.*` imports the implementer claims are verified.
1. **`test_no_phantom_adapter_paths` now passes**: Verify by running `uv run pytest tests/docs/test_doc_accuracy.py::test_no_phantom_adapter_paths --no-cov` (the agent reports xfail dropped from 34 → 33 because this test now passes).
1. **`test_no_prohibited_imports`**: 12 cases should still xfail because Phase 5 only touches adapter READMEs; ACB references in `CHANGELOG.md`, `docs/examples/syntax_demo.py`, etc. (per Phase 4 F1) keep the count high.
...
1. **One commit**: `git log --oneline 516fd95..0b6dc1b` should show exactly one commit.
1. **Author email**: `git log -1 --format='%ae' 0b6dc1b` must be `les@wedgwoodwebworks.com`.
