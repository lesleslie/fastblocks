...
1. README.md:1395 — ACB acknowledgements dropped
1. QWEN.md:7,13 — "Built on ACB" framing removed
1. QWEN.md:30 — "Middleware Communication Protocol" → "Model Context Protocol"
1. QWEN.md:34,38,120 — ACB mentions stripped
1. QWEN.md:53 — `python -m fastblocks serve` replaced with `granian fastblocks.applications:app`
1. RULES.md:154,211 — crackerjack CLI standardized to `uv run crackerjack run`
1. RULES.md:234,371 — fabricated `-x -t` and `--comprehensive` flags removed
1. RULES.md:292 — coverage target text replaced with "Floor: 49.13%"
1. RULES.md:340,361 — `ws://localhost:8675` references removed
1. RULES.md:362 — fabricated MCP tool names removed
1. RULES.md:20-23,37 — typing/docstrings rule scoped to "new code only"
...
1. **API accuracy**: The implementer reports using `resolve_component_async(depends, ...)` from `fastblocks/core/resolver.py`. Verify against the source that this is the canonical public API.
1. **Coverage value**: The brief says 49.13%; pyproject.toml has `--cov-fail-under=49.1324200913242`. The implementer used 49.13% verbatim. Within tolerance — verify the doc text is consistent.
1. **One commit only**: `git log --oneline 41ad715..bf989d6` should show exactly one commit (the amended one).
1. **No out-of-scope edits**: README.md, QWEN.md, RULES.md only. `tests/docs/test_doc_accuracy.py` must NOT be touched (Phase 10 owns the xfail removal). `docs/`, `CHANGELOG.md`, `CLAUDE.md`, `CONTRIBUTING.md` must NOT be touched (Phases 4-6 own those).
1. **Author email**: `git log -1 --format='%ae' bf989d6` must be `les@wedgwoodwebworks.com` (not `.local`).
1. **Concerns addressed**: Concern #1 (resolver API) was addressed by amendment to `bf989d6`. Concerns #2-#6 are documented in the report. Confirm the amendments resolved #1 cleanly and #2-#6 are acceptable observations (not blocking defects).
...
1. The implementer stayed within the worktree (`/Users/les/Projects/fastblocks/.claude/worktrees/doc-audit-2026-08-19/`).
