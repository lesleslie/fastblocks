...
def mock_cli_module(clean_modules: None) -> t.Generator[types.ModuleType]:
    ...
    setattr(mock_typer, "Option", MagicMock())
    setattr(mock_typer, "Annotated", MagicMock())
    ...
    setattr(mock_acb_console, "console", mock_console)
    ...
    setattr(cli_module, "uvicorn", mock_uvicorn)
    setattr(cli_module, "Granian", mock_granian_class)
    setattr(cli_module, "signal", signal)
    setattr(cli_module, "sys", sys)
    setattr(cli_module, "os", os)
    setattr(cli_module, "Path", Path)
    setattr(cli_module, "AsyncPath", mock_anyio_path)
    setattr(cli_module, "execute", MagicMock())
    setattr(cli_module, "asyncio", asyncio)
    setattr(cli_module, "console", mock_console)
    setattr(cli_module, "dump", mock_dump)
    setattr(cli_module, "load", mock_load)
    setattr(cli_module, "fastblocks_path", MagicMock())
    setattr(cli_module, "apps_path", MagicMock())
    setattr(cli_module, "Enum", Enum)
    ...
    setattr(cli_module, "run_args", run_args)
    ...
    setattr(cli_module, "Styles", Styles)
    ...
    setattr(cli_module, "handle_signal", handle_signal)
    ...
    def run_command(docker: bool = False, granian: bool = False) -> None:
        if docker:
            ...
            if granian:
                ...
                cli_module.uvicorn.run(**run_args | dict(host="0.0.0.0", lifespan="on"))  # nosec B104
    ...
    def dev_command(granian: bool = False) -> None:
        ...
        if granian:
            ...
            cli_module.uvicorn.run(**run_args | dict(lifespan="on"))
    ...
    setattr(cli_module, "run", run_command)
    setattr(cli_module, "dev", dev_command)
    setattr(cli_module, "create", create_command)
    setattr(cli_module, "update_settings", update_settings)
    setattr(cli_module, "update_configs", update_configs)

    # Set DEVNULL constant used in the real module
    setattr(cli_module, "DEVNULL", -1)
    ...
    setattr(cli_module, "default_adapters", default_adapters)
...
class TestCLIComprehensive:
    def test_setup_signal_handlers(
        self, ensure_cli_module: None, mocker: MockerFixture
    ) -> None:
    ...
    def test_run_with_docker(self, ensure_cli_module: None) -> None:
        ...
        def test_run(docker: bool = False) -> None:
            if docker:
                mock_execute("docker run -it -ePORT=8080 -p8080:8080 test_app".split())
    ...
    def test_create_command(
        ...
        def test_create(app_name: str, style: Styles, domain: str = "example.com"):
