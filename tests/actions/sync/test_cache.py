...
import pytest
from fastblocks.actions.sync.cache import CacheSyncResult, _clear_cache, sync_cache
...
async def test_sync_records_one_namespace_failure_and_continues() -> None:
    cache = AsyncMock()
    ...
    cache.clear.side_effect = clear_namespace
    result = CacheSyncResult()

    await _clear_cache(cache, ["good", "broken"], SyncStrategy(), result)

    assert result.completed["good"] is True
    assert result.item_errors["broken"] == "namespace failed"
